...
## Usage on Windows
1) Double click the `festival-miner.exe` file
...
2) Start the miner with `./festival-miner`

## Command Line Options

The miner allow to overwrite some of the settings via command line options.
Run `festival-miner --help` to show all available command line options.
...
```
docker run --rm -it -u $(id -u):$(id -g) --name festival/festival-miner -v "$PWD":/mnt festival-miner
docker stop festival-miner
docker run --rm -it -u $(id -u):$(id -g) --name festival/festival-miner -v "$PWD":/mnt festival-miner --config config.txt
```

Debug the docker image by getting inside:

```
docker run --entrypoint=/bin/bash --rm -it -u $(id -u):$(id -g) --name festival/festival-miner -v "$PWD":/mnt festival-miner
```
