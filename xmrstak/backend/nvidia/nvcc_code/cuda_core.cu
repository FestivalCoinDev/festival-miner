		...
		uint32_t startNonce, uint32_t * __restrict__ d_input )
{
	...
	uint32_t tweak1_2[2];
	if (ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_festival)
	{
	...
	for ( i = start; i < end; ++i )
	{
		...
		for ( int x = 0; x < 2; ++x )
		{
            ...
            *( (uint64_t *) t2 ) += sub2 ? ( *( (uint64_t *) t1 ) * *( (uint64_t*) zz ) ) : __umul64hi( *( (uint64_t *) t1 ), *( (uint64_t*) zz ) );
			...
			if (ALGO == cryptonight_festival && sub == 2) {
...
void cryptonight_core_cpu_hash(nvid_ctx* ctx, xmrstak_algo miner_algo, uint32_t startNonce)
{
	...
	else if (miner_algo == cryptonight_festival)
	{
		cryptonight_core_gpu_hash<CRYPTONIGHT_FESTIVAL_ITER, CRYPTONIGHT_FESTIVAL_MASK, CRYPTONIGHT_FESTIVAL_MEMORY/4, cryptonight_festival>(ctx, startNonce);
	}
}
