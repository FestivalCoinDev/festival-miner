...
ENV GIT_REPOSITORY https://github.com/FestivalCoinDev/festival-miner.git
...
RUN apt-get update \
    ...
    && make \
    && cd - \
    && mv /festival-miner/bin/* /usr/local/bin/ \
    && rm -rf /festival-miner \
...
ENTRYPOINT ["/usr/local/bin/festival-miner"]
